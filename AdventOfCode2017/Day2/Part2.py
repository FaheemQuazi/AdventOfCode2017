...
# Day 2 Part 2        #
#######################
# Woah, a quick note appears!
#    I have to admit, this solution is kinda crude.
#    Might have to come back to this one after a while...
#######################
...
for line in lines:
    # split line by tab
    items = line.split('\t')
    # store solution of this line here
    lineSol = 0
    ...
    for i in range(len(items)):
        ...
        for j in range(len(items)):
            ...
            itemB = int(items[j])
            # exit this iteration if itemA and itemB are the same item in the array
            if i == j:
                pass
            # check if the two items divide evenly
            elif itemA % itemB == 0:
                # the division is the solution for this line
                lineSol = (itemA / itemB)
    
    # add this line's solution to the checksum
    checksum += lineSol

# print checksum as integer (otherwise there's a stray decimal)
print(str(int(checksum)))