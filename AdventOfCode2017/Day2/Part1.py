...
# Store the puzzle input here
puzzle = """116	1470	2610	179	2161	2690	831	1824	2361	1050	2201	118	145	2275	2625	2333
...
# the checksum is our result; lets keep it here
checksum = 0;

# split the puzzle by line
lines = puzzle.split('\n')

# loop through all the lines
for line in lines:
    # items are tab-delimited, split by tabs (\t)
    items = line.split('\t')

    # keep largest value in line here
    lv = int(items[0])

    # keep smallest value in line here
    sv = int(items[0])

    # loop through all items in line except first
    # we're using the first item for the baseline comparison; so we exclude it
    for item in items[1:]:
        item = int(item) # comparison needs int

        # check whether current item is larger or smaller than the current largest/smallest item, respectively
        if item > lv:
            lv = item
        elif item < sv:
            sv = item

    # add the difference between largest and smallest value in line to the checksum
    checksum += (lv - sv)
